...
PRODUCT_COPY_FILES += \
    vendor/ninja/prebuilt/common/bin/otasigcheck.sh:install/bin/otasigcheck.sh

# Ninja-specific init file
PRODUCT_COPY_FILES += \
...
PRODUCT_COPY_FILES += \
    vendor/ninja/prebuilt/common/etc/init.d/50selinuxrelabel:system/etc/init.d/50selinuxrelabel
...
PRODUCT_COPY_FILES += \
    ...
    vendor/ninja/prebuilt/common/etc/sysctl.conf:system/etc/sysctl.conf

# Fix Dialer
PRODUCT_COPY_FILES +=  \
    vendor/ninja/prebuilt/common/sysconfig/dialer_experience.xml:system/etc/sysconfig/dialer_experience.xml

# Ninja-specific startup services
PRODUCT_COPY_FILES += \
    vendor/ninja/prebuilt/common/etc/init.d/00banner:system/etc/init.d/00banner \
    vendor/ninja/prebuilt/common/etc/init.d/90userinit:system/etc/init.d/90userinit \
    vendor/ninja/prebuilt/common/bin/sysinit:system/bin/sysinit
...
bootanimation_sizes := $(subst .zip,, $(shell ls vendor/ninja/prebuilt/common/bootanimation))
...
PRODUCT_COPY_FILES += \
    vendor/ninja/prebuilt/common/bootanimation/halfres/$(TARGET_BOOTANIMATION_NAME).zip:system/media/bootanimation.zip
else
PRODUCT_COPY_FILES += \
    vendor/ninja/prebuilt/common/bootanimation/$(TARGET_BOOTANIMATION_NAME).zip:system/media/bootanimation.zip
endif
endif
...
NINJA_POSTFIX := -$(shell date +"%Y%m%d-%H%M")
...
EXTENDED_POST_PROCESS_PROPS := vendor/ninja/tools/ninja_process_props.py
