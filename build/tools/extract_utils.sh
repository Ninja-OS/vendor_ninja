...
function setup_vendor() {
    ...
    if [ ! -d "$NINJA_ROOT" ]; then
        echo "\$NINJA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$NINJA_ROOT/$OUTDIR" ]; then
        mkdir -p "$NINJA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$NINJA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$NINJA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$NINJA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$NINJA_ROOT"/vendor/ninja/build/tools/smali/baksmali.jar
        export SMALIJAR="$NINJA_ROOT"/vendor/ninja/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$NINJA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$NINJA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$NINJA_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$NINJA_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$NINJA_ROOT"/vendor/ninja/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$NINJA_ROOT"/"$OUTDIR"/radio
